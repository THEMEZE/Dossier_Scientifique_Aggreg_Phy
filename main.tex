...
On dit alors que le système est \emph{\textsf{ergodique}} ou
\emph{\textsf{chaotique}} : au cours du temps, il explore efficacement son
espace des phases, ce qui permet la thermalisation et l’apparition d’un
comportement universel à grande échelle.
        ...
        \begin{scope}[shift={(0,0)}]
            ...
            \node (a) at (0,0) {
                \animategraphics[
                    ...
                    %buttonsize=1em,
                    poster=first
                ]{60}{figures/00_PreRemInt/frames_2D_positions_animation/frame_}{00001}{00010}
            ...
            \node[draw = none  , rectangle , right = 0.1cm of b  ] (c)   {
                ...
                ]{60}{figures/00_PreRemInt/frames_2D_velocity_distribution/frame_}{00001}{00010}
            };%
                ...
                \begin{scope}[shift={(0,0)}]
                    ...
                    \node (a) at (0,0) {
                        \animategraphics[
                            ...
                            %buttonsize=1em,
                            poster=first
                        ]{60}{figures/00_PreRemInt/frames_1D_positions_animation/frame_}{00001}{00010}
                    ...
                    \node[draw = none , rectangle , shift={(-2 , 0)},  below = 0.5cm of a] (b)  {
                        ...
                        ]{60}{figures/00_PreRemInt/frames_Newton/frame_}{00001}{00013} }; %}
...
À l’état fondamental, le facteur d’occupation $\nu$ vaut $1$ pour $|\theta|<\theta_F$ et $0$ ailleurs (cf. fig.~\ref{fig:eqBA}), où $\theta_F$
...
\begin{tikzpicture}[overlay]
    ...
    \draw[  opacity = 1 ]
    (Xi) edge[line width=0.2ex ,color = colorThree ,arrows = {Computer Modern Rightarrow[sharp ,length=0.1cm]-},  , out = -90 , in = 180  ]node[pos=1, draw = none , right=0.00cm , rectangle  , shift = {(0.00cm , -0.0cm)} , text width=6.5cm,align=center ]{\color{colorThree} \fontsize{4pt}{2pt} \linespread{0.2} \selectfont \baselineskip=4pt \bfseries $ \displaysyle \Xi[A,J] = \int d \vex{x} \, \exp\left({-\frac{1}{2} \langle\vec{x}, A \vec{x} \rangle + \langle \vec{J} , \vec{x} \rangle  }\right) = \frac{\exp\left({\frac{1}{2} \langle\vec{J}, A \vec{J} \rangle  }}{ \sqrt{\det \left ( \frac{A}{2\pi} \right ) } \right ) } $ }($(Xi) + (0.5cm,-1.0cm)$)
    ;
...
{woodChene}
{
    ...
    \textbf{Niveau : Lycée L1–L3 / CPGE — Thèmes : physique expérimentale, ordres de grandeur, interaction lumière-matière.}
    ...
    \begin{enumerate}[label =\textbf{\arabic*.}, ref=\arabic*]
        ...
        \item {\bf \textsf{Ordres de grandeur}}. {\bf \textsf{Niveau :}} {\em Lycée-Licence}.
        ...
        \item {\bf \textsf{Piégeage des atomes.}} . {\bf \textsf{Niveau :}} {\em Licence}.
        ...
        \item {\bf \textsf{Approche expérimentale concrète}}. {\bf \textsf{Niveau :}} {\em Lycée-Licence}.
              Présentation (ou visite) d’un Laboratoire Charle Fabry d’atomes froids, avec une immersion dans les dispositifs expérimentaux :
              \begin{itemize}
...
\EncadreDeux
{Activité 2}
{{\bf \textsf{Activité 2 : Effets colectifs et Ergodicité}}}
{woodChene}
{
...
\textbf{Objectif :}
%Cette activité a pour objectif d’introduire concrète à plusieurs notionsimportantes de physique statistique comme  l’{\bf ergodicité} (l’oubli des conditionsinitiales et l’émergence de l’équilibre thermodynamique), le {\bf chaos} et de {\bf thermalisation} en les comparant avec des situation d'{\bf intégrabilité} à partir de modèles simples. Elle permet de montrer que le comportement collectif d’un système dépend fortement de la présence ou non de quantités conservées.

Cette activité vise à introduire de manière concrète plusieurs concepts
fondamentaux de la physique statistique hors équilibre, notamment l’{\bf
ergodicité} (perte d’information sur les conditions initiales et relaxation
vers l’équilibre thermodynamique), le {\bf chaos dynamique} et la {\bf
        thermalisation}. Ces notions sont mises en perspective à travers leur
confrontation avec des systèmes {\bf intégrables}, à partir de modèles jouets
explicitant le rôle des lois de conservation. On met ainsi en évidence que la
dynamique collective d’un système dépend de manière cruciale de la structure de
ses constantes du mouvement et de leur suffisance pour contraindre l’espace des
phases.

%L’activité constitue également une introduction concrète à plusieurs notionsimportantes de physique statistique, comme l’{\bf ergodicité}, l’oubli des conditions initiales et l’émergence de l’équilibre thermodynamique. Elle permet enfin d’ouvrir vers des thématiques de recherche actuelles, comme le ({\bf \textsf{GGE}}) et ({\bf \textsf{GHD}}).

\begin{enumerate}[label =\textbf{\arabic*.}, ref=\arabic*]

    \item {\bf \textsf{Modélisation de gaz de sphères dures.}} \textbf{Niveau :} {\em L2--L3}. \textbf{Notion:} {\em Effets colectifs}.

          Simulation numérique d’un gaz de sphères dures en interaction élastique, en une
          et deux dimensions (cf figs~ \ref{fig:hard_spheres_1D_animation},
          \ref{fig:hard_spheres_2D_animation} et \ref{fig:Pi12} b)) pour faire un premère
          aperçu de la notion d'{\bf intégrabilité} et d'{\bf ergodicité} et les effets
          du diametres des sphères dures sur leur vitesse effective. Simulation de
          trajectoires 1D de sphères dures en interaction élastique. Cette activité peut
          être réalisée sous Python pour des étudiants de L3 ou de CPGE, ou être
          présentée sous forme de démonstration numérique pour des niveaux plus
          élémentaires .
          ...
          %     (Après les cours de mécanique du point.) Étude du problème à deux corps à partir du passage au référentiel du centre de masse, permettant de ramener le système à un problème effectif à une particule. Ajouter les éffets du diametre finie de sphère dure.

          %       (Après les cours de mécanique quantique.) Comparer l’équation de {\bf \textsf{Schrödinger}} à une particule avec la version $N=2$ du modèle de {\bf \textsf{(LL)}} (eq.~\eqref{eq.LL}). Ramener  cette dernière équation, problème à deux corps en un problème à une particule (comment en mécanique classique) et en deduire les solution stationnaire pour $N=2$. Comparer cette solution avec l'état \eqref{eq.StationaryState}.

          %       Dans le cas périodique, on peut ensuite introduire la quantification des
          %       nombres d’onde pour $N=1$ puis $N=2$ et comparer avec les équations {\bf \textsf{(BA)}}(eq.~\eqref{eq.BA}).
          %       Que devien (eq.~\eqref{eq.BA}) pour une gaz parfaits (sans intéraction et dans diamètre). Conclure sur l'effat intéractions collectifs sur les états accessibles.

    \item {\bf \textsf{Problème à deux corps et interactions.}}  \textbf{Niveau :} {\em L2--L3 (avec accompagnement sur les distributions)} . \textbf{Notion:} {\em Effets colectifs}.
          (Après les cours de mécanique du point.) Étude du problème à deux corps via le passage au référentiel du centre de masse, permettant de ramener la dynamique à un problème effectif à une particule soumise à une interaction centrale. On introduit également les effets d’un potentiel de sphères dures de diamètre fini.

          (Après les cours de mécanique quantique.) Comparaison entre l’équation de {\bf \textsf{Schrödinger}} à une particule et la version à $N=2$ du modèle de {\bf \textsf{(LL)}} (eq.~\eqref{eq.LL}). On montre comment cette dernière se ramène, dans le cas $N=2$, à un problème effectif à une particule, de manière analogue au traitement classique, et on en déduit les états stationnaires. On compare ensuite ces solutions avec l’état \eqref{eq.StationaryState}.

          Dans le cas périodique, on introduit la quantification des nombres d’onde pour
          $N=1$ puis $N=2$, et on compare avec les équations de {\bf \textsf{(BA)}}
          (eq.~\eqref{eq.BA}). On discute enfin la limite de gaz parfait (absence
          d’interaction et diamètre nul), et l’on met en évidence l’impact des
          interactions sur la structure des états accessibles et sur la dynamique
          collective.
    ...
    \item {\bf \textsf{Ouverture mathématique : matrices aléatoires de Wigner.}} \textbf{Niveau :} {\em CPGE (MP)}

          Étude numérique des valeurs propres de matrices symétriques aléatoires à coefficients gaussiens et mise en évidence de la loi du demi-cercle de Wigner.\textbf{Lien :} Distribution de rapidité vs valeur propre.

    \item {\bf \textsf{Comparaison hydrodynamique.}}  \textbf{Niveau :} {\em L2--L3} . \textbf{Notion:} {\em Effets colectifs \& Ergodicité }.
          ...
          \begin{center}
              ...
              \begin{minipage}{0.45\linewidth}
                  où la densité de particules $n = q_{[1]}$, la vitesse moyenne du fluide
                  $u = q_{[\theta]}/q_{[1]}$, la pression cinétique par
                  \(
                  ...
                  E = \frac{n u^2}{2} + nV + ne_{\rm int},
                  \)
                  avec l’énergie interne par particule
                  \(
                  e_{\rm int} = q_{[\theta^2/2]}
                  \)
                  .
    ...
    \item {\bf \textsf{Fluctuations et ensembles statistiques.}} \textbf{Niveau :} {\em L2--L3} . \textbf{Notion:} {\em Ergodicité }
