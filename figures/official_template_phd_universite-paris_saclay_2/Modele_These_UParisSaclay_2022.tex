...
\put(-120,-743){\rotatebox{90}{NNT : 2025UPASP137}}
\end{picture}
...
\color{black}
%\Large{
    \textit{Study of the out-of-equilibrium dynamics\\
    of one-dimensional Bose gases}
    %}
} 
\\
%*****************************************************
...
%\small 
École doctorale n$^{\circ}$572, Ondes et Matières\\
%\small 
...
%Thèse préparée au \textbf{Laboratoire Charles Fabry} \\
Thèse préparée dans l'unité de recherche \textbf{Université Paris-Saclay},\\
\textbf{Institut d'Optique Graduate School}, \textbf{CNRS},\\
\textbf{Laboratoire Charles Fabry}, 91127, Palaiseau, France,\\
%(Université Paris-Saclay, CNRS, Institut d’Optique Graduate School),\\
sous la direction de \textbf{Isabelle BOUCHOULE}, Directrice de thèse CNRS. \\
}
\vspace{10mm}
...
\color{black}    
\fontsize{12}{12}\selectfont
\textbf{Thèse soutenue à Paris-Saclay, le 4 novembre 2025, par}}\\[5mm]
\bigskip

{   
\fontsize{20}{12}\selectfont
%\Large {
    \color{Prune} \textbf{Guillaume THÉMÈZE}
...
%\textbf{Laurent VERNAC} &   Président ou Présidente\\ 
%Titre, Affiliation & \\
{\textbf{Dmitry PETROV}} &  {Président} \\ 
{Directeur de recherche (LPTMS)}   &   \\ 
{\textbf{Laurent VERNAC}} &  Rapporteur \\ 
{ Professeur, Universités Sorbonne (LPL)}  &   \\ 
{\textbf{Nicolas CHERRORET}} &  {Rapporteur} \\ 
{Chargé de recherche (LKB)}  &   \\ 
{\textbf{Martin ROBERT DE SAINT VINCENT} }&  {Examinateur}\\ 
{ Chargé de recherche (LPL)}  &   \\ 
\end{tabular} \\
}
...
{\color{Prune} %\scriptsize
\fontsize{9}{12}\selectfont 
{Membres du jury sans voix délibérative}} \\
\vspace{2mm}
%\scriptsize
{\color{black}
\fontsize{10}{12}\selectfont
\begin{tabular}{|p{7cm}l}
\arrayrulecolor{Prune}
{\textbf{Jérôme DUBAIL}} &  {Membre invité} \\%[0.5ex] 
{Chargé de Recherche, (CESQ)}  &   \\ 
\end{tabular} 
}
...
%*****************************************************
\includegraphics[height=2.45cm]{logo_usp_EDOM.png}
\vspace{1cm}
%*****************************************************
...
\textbf{Titre:} Étude de la dynamique hors équilibre des gaz de Bosons unidimensionnels

\noindent \textbf{Mots clés:} Quantique - Intégrabilité - Unidimensionnel - Boson - Dynamique

\vspace{-.5cm}
\begin{multicols}{2}
\noindent \textbf{Résumé:} {\fontsize{11pt}{6pt}\selectfont 
Cette thèse porte sur l’étude expérimentale et théorique des gaz quantiques unidimensionnels hors équilibre, réalisée avec des atomes ultra-froids de rubidium 87 piégés sur une puce atomique. Ce dispositif permet un confinement transverse fort et un contrôle longitudinal précis, ouvrant la voie à l’exploration de situations exotiques comme l’analogue quantique du pendule de Newton.

\medskip

Dans ce régime, les interactions sont décrites par le modèle intégrable de Lieb-Liniger, résolu par la méthode de Bethe Ansatz. Les systèmes ainsi obtenus ne se thermalisation pas selon Gibbs, mais via l’ensemble de Gibbs généralisé (GGE). J’ai initié un travail théorique sur le calcul des fluctuations d’observables, nécessaire pour tester cette description, sans toutefois mener de mesures expérimentales correspondantes.

\medskip

L’hydrodynamique généralisée (GHD) complète le GGE en décrivant la dynamique à grande échelle. Dans ce cadre, nous avons étudié des dynamiques de type « problème de Riemann », et développé expérimentalement un dispositif de micro-miroirs (DMD) pour façonner les potentiels et sonder localement la distribution de rapidité, ouvrant la voie à des situations hors équilibre contrôlées.

\medskip

Enfin, nous avons exploré des alternatives au confinement magnétique sur puce, en particulier la mise en place d’un piège dipolaire optique offrant une flexibilité accrue dans l’ingénierie de potentiels longitudinaux.

\medskip

En résumé, cette thèse combine avancées expérimentales et théoriques sur la préparation et la manipulation de gaz bosoniques intégrables, la mise en œuvre d’outils de mesure locaux, et l’articulation avec les cadres modernes de relaxation et de dynamique (GGE et GHD).
%Cette thèse s’inscrit dans le cadre de l’étude des gaz quantiques unidimensionnels hors équilibre, réalisée au sein du laboratoire LCF, groupe Gaz quantiques, équipe Puce. Nous utilisons des gaz ultra-froids d’atomes de rubidium 87 piégés sur une puce atomique, permettant de réaliser un confinement transversal fort et un confinement longitudinal ajustable à l’aide de micro-fils. Ce dispositif permet d’explorer des situations exotiques, telles que l’analogue quantique d’un pendule de Newton.

%\medskip

%Dans ce régime unidimensionnel, les interactions de contact entre atomes sont décrites par le modèle intégrable de Lieb-Liniger, résoluble par la méthode de Bethe Ansatz. L’état d’un tel système est caractérisé par une distribution de rapidités, associée aux vitesses des quasi-particules. La présence d’une infinité de charges conservées fait de ces systèmes intégrables des cas particuliers où la thermalisation standard de type Gibbs ne s’applique pas. À la place, leur relaxation est décrite par l’ensemble généralisé de Gibbs (GGE). Pour tester expérimentalement cette description, la mesure d’observables moyennes ne suffit pas : il est nécessaire d’accéder également aux fluctuations. J’ai initié un travail théorique sur le calcul de telles fluctuations, sans toutefois réaliser de mesure expérimentale correspondante au cours de cette thèse.

%\medskip

%Un autre cadre théorique pertinent est l’hydrodynamique généralisée (GHD), qui complète le GGE en décrivant la dynamique hors équilibre des gaz intégrables à grande échelle. Dans ce contexte, nous avons étudié des dynamiques de type « problème de Riemann », où l’on prépare initialement un système avec des atomes d’un côté et du vide de l’autre. Pour approcher expérimentalement cette configuration, nous avons optimisé la préparation de gaz longs et homogènes, avant de sélectionner localement une région à l’aide d’un dispositif de micro-miroirs (DMD), permettant de sculpter le profil lumineux appliqué aux atomes et de sonder localement la distribution de rapidité. Ce dispositif ouvre également la voie à la création de situations hors équilibre contrôlées.

%\medskip

%Enfin, afin d’élargir les possibilités de préparation de potentiels longitudinaux, nous avons exploré des alternatives au confinement magnétique sur puce. En particulier, j’ai initié la mise en place d’un piège dipolaire optique, permettant d’obtenir une flexibilité accrue dans l’ingénierie des géométries de piège, allant au-delà des configurations harmoniques, double puits ou quartiques déjà accessibles sur puce.

%\medskip

%En résumé, cette thèse combine le développement expérimental et théorique pour sonder la dynamique hors équilibre des gaz de bosons unidimensionnels. Les avancées portent à la fois sur la préparation et la manipulation de systèmes quasi-intégrables, sur la mise en place d’outils de mesure locaux originaux, et sur le lien avec les cadres théoriques modernes de relaxation et de dynamique des systèmes intégrables (GGE et GHD).
...
Finally, we explored alternatives to chip-based magnetic confinement, in particular the implementation of an optical dipole trap providing increased flexibility in engineering longitudinal potentials.
...
%This thesis is devoted to the study of out-of-equilibrium one-dimensional quantum gases, carried out at the LCF laboratory, Quantum Gases group, Chip team. We use ultracold rubidium-87 atoms trapped on an atom chip, which provides a strong transverse confinement and a tunable longitudinal confinement via micro-wires. This platform allows us to explore exotic situations, such as the quantum analogue of a Newton’s cradle.

%\medskip	

%In this one-dimensional regime, contact interactions between atoms are described by the integrable Lieb–Liniger model, solvable through the Bethe Ansatz. The state of such a system is characterized by a distribution of rapidities, associated with the velocities of quasi-particles. Due to the existence of an infinite set of conserved charges, integrable systems are special cases where standard Gibbs thermalization does not apply. Instead, their relaxation is described by the Generalized Gibbs Ensemble (GGE). To test this description experimentally, the measurement of mean observables is not sufficient: one also needs access to fluctuations. I initiated a theoretical study of such fluctuations, although no corresponding experimental measurement was carried out during this thesis.

%\medskip

%Another relevant theoretical framework is Generalized Hydrodynamics (GHD), which complements the GGE by describing the large-scale out-of-equilibrium dynamics of integrable gases. In this context, we investigated dynamics of the “Riemann problem” type, where a system is initially prepared with atoms on one side and vacuum on the other. To approximate this configuration experimentally, we optimized the preparation of long and homogeneous gases, before locally selecting a region using a digital micromirror device (DMD). This tool makes it possible to shape the light profile applied to the atoms and to probe locally the rapidity distribution, thereby enabling the engineering of controlled out-of-equilibrium situations.

%\medskip

%Finally, to broaden the possibilities for preparing longitudinal potentials, we explored alternatives to chip-based magnetic confinement. In particular, I initiated the implementation of an optical dipole trap, which offers enhanced flexibility in engineering trapping geometries, beyond the harmonic, double-well, or quartic configurations already accessible on the chip.

%\medskip

%In summary, this thesis combines experimental and theoretical developments to probe the out-of-equilibrium dynamics of one-dimensional bosonic gases. The advances achieved concern both the preparation and manipulation of quasi-integrable systems, the implementation of original local probing techniques, and the connection with modern theoretical frameworks for the relaxation and dynamics of integrable systems (GGE and GHD).
}
\end{multicols}
\end{mdframed}
